...
class Booking_model extends CI_Model
{
  ...
  function getSlots($Mode, $Building)
  {
    $this->db->select('*');
  ...
  {
    $this->db->select('*, booking.BookingRefNo AS BookingRefNo, bookingmode.Mode AS BookMode, slottypes.Type AS SlotType, booking.CreatedOn AS BookedOn, booking.CheckIn AS CheckIn, booking.CheckOut AS CheckOut');
    $this->db->from('booking');
    ...
    if ($filter == 'Today') {
      if (in_array($this->session->userdata('Role'), array(5, 6))) {
        $this->db->where('DATE(booking.CheckIn)', date('Y-m-d'));
        $this->db->where('booking.BuildingName', 'ICC1 - Admin');
      } else if (in_array($this->session->userdata('Role'), array(7, 8))) {
        $this->db->where('DATE(booking.CheckIn)', date('Y-m-d'));
        $this->db->where('booking.BuildingName', 'ICC2 - Admin');
      } else {
        $this->db->where('DATE(booking.CheckIn)', date('Y-m-d'));
      }
  ...
  function SaveMultiVendorBooking($data)
  {
    $this->db->insert('booking', $data);
    $booking_id = $this->db->insert_id();
    if (!empty($booking_id)) {
      $ds['SlotID'] = $data['SlotID'];
      $ds['BookingUID'] = $booking_id;
      $ds['SlotType'] = $data['SlotType'];
      $ds['VNo'] = $data['VNo'];
      $ds['CheckIn'] = $data['CheckIn'];
      $ds['CheckOut'] = $data['CheckOut'];
      $ds['CreatedBy'] = $this->session->userdata('UserUID');
      $this->db->insert('bookedslots', $ds);
      return $booking_id;
    } else {
      return 0;
    }
  ...
  function getVendorBooking($VRefNo)
  {
    $this->db->select('*');
    $this->db->where('VendorBookingRefNo', $VRefNo);
    $this->db->where('ActualCheckOut', Null);
  ...
  function updateCheckout($VRefNo, $data)
  {
    $this->db->where('VendorBookingRefNo', $VRefNo);
    $this->db->where('ActualCheckOut', Null);
    $this->db->update('booking', $data);
